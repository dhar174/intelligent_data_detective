...
*   **`CLASS_TO_AGENT` Mapping Typo**
    *   **Context:** `CLASS_TO_AGENT` dict maps `VisualizationResults: "viz_join"`.
    *   **Description:** If an agent creates a `VisualizationResults` object, the supervisor routes to `viz_join`. However, `viz_join` is a synthesizer node that expects `viz_results` (a list of dicts) generated by `viz_worker`. The supervisor should probably route to `"visualization"` (the orchestrator node) or `"viz_worker"` directly if it expects to kick off the visualization process. Routing to `viz_join` without tasks will fail or produce empty results.
