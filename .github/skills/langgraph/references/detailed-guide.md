...
# In this repository, use MyChatOpenai (AGENTS.md:77) to ensure custom payload formatting
try:
    from idd_core import MyChatOpenai
except ImportError:
    from langchain_openai import ChatOpenAI as MyChatOpenai
...
llm = MyChatOpenai(model="gpt-4o").bind_tools(tools)
...
# SQLite checkpointer (note: SqliteSaver.from_conn_string is a context manager)
with SqliteSaver.from_conn_string("agent_state.db") as memory:
    # Compile with checkpointer inside the context manager
