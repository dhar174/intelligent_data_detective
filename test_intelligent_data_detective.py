...
class AnalysisConfig(BaseModel):
    ...
    default_visualization_style: str = Field(
        "seaborn-v0_8-whitegrid",
        description="Default style for matplotlib/seaborn visualizations.",
    )
    report_author: Optional[str] = Field(
        None, description="Author name to include in generated reports."
    )
    datetime_format_preference: str = Field(
        "%Y-%m-%d %H:%M:%S",
        description="Preferred format for datetime string representations.",
    )
    large_dataframe_preview_rows: int = Field(
        5, description="Number of rows for previewing large dataframes."
    )
...
class CleaningMetadata(BaseModel):
    ...
    data_description_after_cleaning: str = Field(
        description="Brief description of the dataset after cleaning."
    )
...
class InitialDescription(BaseModel):
    ...
    data_sample: Optional[str] = Field(
        description="Sample of the data (first few rows)."
    )
...
class AnalysisInsights(BaseModel):
    ...
    correlation_insights: str = Field(
        description="Key correlation insights identified."
    )
    anomaly_insights: str = Field(
        description="Anomalies or interesting patterns detected."
    )
    recommended_visualizations: list[str] = Field(
        description="List of recommended visualizations to illustrate findings."
    )
    recommended_next_steps: Optional[List[str]] = Field(
        None,
        description="List of recommended next analysis steps or questions to investigate based on the findings.",
    )
...
class GetDataParams(BaseModel):
    ...
    index: Union[int, List[int], Tuple[int, int]] = Field(
        ...,
    ...
    columns: Union[str, List[str]] = Field(
        "all",
        description="A string (single column), a list of strings (multiple columns), or 'all' for all columns (default: 'all').",
    )
    cells: Optional[List[CellIdentifier]] = Field(
        None,
        description="A list of cell identifier objects, each specifying a 'row_index' and 'column_name'.",
    )
    ...
    def validate_index(cls, values):
        ...
        if isinstance(index, tuple) and len(index) != 2:
            raise ValueError(
                "Invalid tuple length for 'index'. Must be a 2-tuple for range."
            )
        if isinstance(index, list) and not all(isinstance(i, int) for i in index):
            raise ValueError(
                "Invalid list elements for 'index'. Must contain only integers."
            )
        return values
...
class TestPydanticModels(unittest.TestCase):
    ...
    def test_analysis_config_valid(self):
        ...
        custom_config = AnalysisConfig(
            report_author="Test Author", large_dataframe_preview_rows=10
        )
    ...
    def test_cleaning_metadata_required_fields(self):
        ...
        metadata = CleaningMetadata(
            steps_taken=["step1", "step2"], data_description_after_cleaning="Clean data"
        )
...
class TestDataFrameRegistry(unittest.TestCase):
    ...
    def test_get_raw_path_from_id(self):
        ...
        df_id = self.registry.register_dataframe(
            self.sample_df1, "df_custom", raw_path=raw_path_str
        )
    ...
    def test_get_dataframe_load_if_not_exists(self):
        ...
        df_id_load = self.registry.register_dataframe(
            df=None, df_id="df_load", raw_path=str(self.test_csv_path)
        )
    ...
    def test_get_dataframe_load_if_not_exists_file_not_found(self):
        ...
        df_id_missing = self.registry.register_dataframe(
            df=None,
            df_id="df_missing",
            raw_path=str(Path(self.temp_dir) / "non_existent.csv"),
        )
    ...
    def test_registry_capacity_limit(self):
        ...
        small_registry.register_dataframe(self.sample_df1, "df1")
        small_registry.register_dataframe(self.sample_df2, "df2")
...
class TestDataValidation(unittest.TestCase):
    ...
    def test_dataframe_with_all_nan_column(self):
        ...
        df_with_nan = pd.DataFrame(
            {"all_nan": [None, None, None], "some_data": [1, 2, 3]}
        )
