...
- Notable calls (39 unique): AfterValidator, ConfigDict, Field, PlanStep.model_validate, PrivateAttr, ValueError, __init__, _norm, _triplet_from_raw, any, base64.b64decode, bool, d.get, dedup_list.sort, dict, field_validator, get, hasattr, info.data.get, int ...
- Preview:
...
- Notable calls (42 unique): DataFrameRegistry, Field, OrderedDict, PathlibPath, df.to_csv, df.to_json, df.to_parquet, df.to_pickle, expanduser, get_global_df_registry, info.get, isinstance, len, list, os.path.exists, path.exists, path.is_file, path.parent.exists, path.parent.mkdir, path.suffix.lower ...
- Preview:
  import threading
...
- Notable calls (28 unique): _LOG.info, _is_df, _to_jsonable, decode, df.head, fn, getattr, inspect.signature, int, isinstance, join, json.dumps, len, list, logging.getLogger, map, math.ceil, max, repr, s.count ...
- Preview:
  import json
  import math
  import logging
...
- Type: code
- Line count: 665
- AST parse error: expected an indented block after function definition on line 229 (<unknown>, line 230)
- Preview:
