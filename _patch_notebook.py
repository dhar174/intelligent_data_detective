...
def main():
    ...
    if "kagglehub" in src48 or "KaggleHub" in src48 or "kaggle" in src48.lower():
        ...
        'df_name = "sample_dirty"' in src48
        and "global_df_registry.register_dataframe" in src48
    ):
        if "IDD_SAMPLE_DATASET" not in src48:
            ...
            if c48["cell_type"] == "code":
                ...
                c48["execution_count"] = None
            print(
                "✅ Cell idx 48: upgraded dataset preparation with IDD_SAMPLE_DATASET switch"
            )
        else:
            print(
                "ℹ️  Cell idx 48 already prepares deterministic datasets with IDD_SAMPLE_DATASET switch"
            )
    else:
        print(
            f"⚠️  Cell idx 48 does not look like Kaggle cell. First 100 chars: {src48[:100]}"
        )
        print(
            "   Searching for a Kaggle download cell, but preserving import/bootstrap cells."
        )
        ...
        for i, c in enumerate(cells):
            ...
            print(
                "ℹ️  No Kaggle download cell found; leaving dataset preparation unchanged"
            )
    ...
    if W13Y_GUARD not in src7 and "def persist_to_drive(" in src7:
        ...
        if old in src7:
            ...
            if old_copy in src7:
                ...
                c7["execution_count"] = None
                print(
                    "✅ Cell idx 7: W13Y-HTML-ROOT-IMAGES patched promoted report HTML links"
                )
            else:
    ...
    if OLD_BOOL_OR in src7_after:
        ...
        if c7.get("cell_type") == "code":
            ...
            c7["execution_count"] = None
        print(
            "✅ Cell idx 7: fixed bool_or reducer (None-safe) — root cause of double data_cleaner run"
        )
    else:
        ...
        for _ci, _cell in enumerate(cells):
            ...
            if OLD_BOOL_OR in _src:
                ...
                print(
                    f"✅ Cell idx {_ci}: fixed bool_or reducer (found outside cell 7)"
                )
                break
        else:
            print(
                "⚠️  bool_or reducer fix: 'from operator import add, or_ as bool_or' not found"
            )
    ...
    for idx, cell in enumerate(cells):
        ...
        if old_field in src:
            ...
            print(
                f"⚠️  Fix B1: 'last_agent_id: Optional[AgentId]' not found in State class cell {idx}"
            )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if "# Fix AR-3" in src:
            print(
                f"ℹ️  Cell idx {idx}: Fix AR-3 (report_paths reducer) already applied"
            )
            fixar3_patched = True
            break
        ...
        if old_ann in src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: Fix AR-3 — report_paths reducer → safe dict-merge lambda"
            )
            fixar3_patched = True
        else:
    ...
    for idx, cell in enumerate(cells):
        ...
        _sig_try_pat = _re_qdf.compile(
            _re_qdf.escape(NEW_SIG) + r"\s*try:", _re_qdf.DOTALL
        )
        _match = _sig_try_pat.search(new_src)
        if _match:
            new_src = (
                new_src[: _match.start()]
                + NEW_SIG
                + "\n"
                + EXTRACT_BLOCK
                + "    try:"
        ...
        if new_src != src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: query_dataframe patched — explicit params arg + extraction logic"
            )
            qdf_patched = True
        break
    if not qdf_patched:
    ...
    for idx, cell in enumerate(cells):
        ...
        src = join_source(cell["source"])
        if (
            "def initial_analysis_node" not in src
            or "initial_analysis_agent.invoke" not in src
        ):
            continue
        if "_safe_initial_analysis_invoke" in src:
            print(
                f"ℹ️  Cell idx {idx}: initial_analysis_node already has safe-invoke patch"
            )
            ia_patched = True
            break
        new_src = src
        ...
        if new_src != src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: initial_analysis_node patched (safe invoke + recursion cap=80 + recovery)"
            )
            ia_patched = True
        else:
            checks = [
            ...
            for needle, label in checks:
                if needle not in src:
                    print(
                        f"⚠️  Cell idx {idx}: initial_analysis patch - '{label}' not found"
                    )
            if new_src == src:
                print(
                    f"⚠️  Cell idx {idx}: initial_analysis_node patch - no replacements made"
                )
        break
    if not ia_patched:
    ...
    for idx, cell in enumerate(cells):
        ...
        if new_src != src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: data_cleaner_node patched (safe invoke + recursion cap=160 + force flags)"
            )
            dc_patched = True
        break
    if not dc_patched:
        print(
            "⚠️  data_cleaner_node patch: target cell not found or no replacements made"
        )
    ...
    for idx, cell in enumerate(cells):
        ...
        AN_REROUTE_OLD_2 = '    if isinstance(cm, CleaningMetadata) and (cm.data_description_after_cleaning or "").strip() == "":'
        AN_REROUTE_NEW_2 = (
        ...
        if AN_REROUTE_OLD_1 in new_src:
            ...
            if AN_REROUTE_OLD_2 in new_src:
                new_src = new_src.replace(
                    AN_REROUTE_OLD_2, AN_CM_SYNTH + AN_REROUTE_NEW_2, 1
                )
            print(f"  ✅ analyst_node: re-route guards patched")
        else:
            print(
                f"  ⚠️  analyst_node: re-route guard pattern not found — skipping guard patch"
            )
        ...
        if new_src != src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: analyst_node patched (safe invoke + recursion cap=50 + recovery + reroute guards)"
            )
            an_patched = True
        else:
            ...
            for needle, label in checks:
                if needle not in src:
                    print(
                        f"⚠️  Cell idx {idx}: analyst_node patch - '{label}' not found"
                    )
            if new_src == src:
                print(
                    f"⚠️  Cell idx {idx}: analyst_node patch - no replacements made (check targets above)"
                )
        break
    if not an_patched:
    ...
    for idx, cell in enumerate(cells):
        ...
        src = join_source(cell["source"])
        if (
            "def report_packager_node" not in src
            or "report_packager_agent.invoke" not in src
        ):
            continue
        if "_safe_report_packager_invoke" in src:
            print(
                f"ℹ️  Cell idx {idx}: report_packager_node already has safe-invoke patch"
            )
            rp_patched = True
            break
        new_src = src
        ...
        if new_src != src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: report_packager_node patched (safe invoke + recursion cap=120 + recovery)"
            )
            rp_patched = True
        else:
            checks = [
            ...
            for needle, label in checks:
                if needle not in src:
                    print(
                        f"⚠️  Cell idx {idx}: report_packager patch - '{label}' not found"
                    )
            if new_src == src:
                print(
                    f"⚠️  Cell idx {idx}: report_packager_node patch - no replacements made"
                )
        break
    if not rp_patched:
    ...
    for idx, cell in enumerate(cells):
        ...
        if "# Fix AB" in src:
            print(
                f"i  Cell idx {idx}: Fix AB (report_packager rg_vars keys) already applied"
            )
            fixAB_patched = True
            break
        _AB_OLD = (
        ...
        if _AB_OLD in src:
            ...
            cell["execution_count"] = None
            print(
                f"OK Cell idx {idx}: Fix AB applied — report_packager rg_vars gets cleaning_metadata + visualization_results"
            )
            fixAB_patched = True
        else:
    ...
    for idx, cell in enumerate(cells):
        ...
        if "# Fix AC" in src:
            print(
                f"i  Cell idx {idx}: Fix AC (report_draft use-last reducer) already applied"
            )
            fixAC_patched = True
            break
        ...
        if _AC_OLD in src:
            ...
            cell["execution_count"] = None
            print(
                f"OK Cell idx {idx}: Fix AC applied — report_draft gets use-last reducer"
            )
            fixAC_patched = True
        else:
    ...
    for idx, cell in enumerate(cells):
        ...
        if "# Fix AD" in src:
            print(
                f"i  Cell idx {idx}: Fix AD (route_to_writer/write_output_to_file END fix) already applied"
            )
            fixAD_patched = True
            break
        new_src = src
        ...
        if _AD_OLD_WOF in new_src:
            ...
            print(
                f"W  Fix AD: write_output_to_file fallback pattern not found in cell {idx}"
            )
        if new_src != src:
            cell["source"] = new_src
            cell["outputs"] = []
            cell["execution_count"] = None
            print(
                f"OK Cell idx {idx}: Fix AD applied — routing/write functions now reach END"
            )
            fixAD_patched = True
        break
    ...
    def _inject_supervisor_shortcut(src):
        ...
        indent_match = _re4.search(
            r"^([ \t]*)def supervisor_node\(state", src, _re4.MULTILINE
        )
        if not indent_match:
            return src, False
    ...
    for idx, cell in enumerate(cells):
        ...
        if "PATCH: force analyst routing" in src:
            print(
                f"ℹ️  Cell idx {idx}: supervisor_node already has analyst-routing patch"
            )
            sup_patched = True
            break
        ...
        if changed:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: supervisor_node patched — force analyst routing after cleaning"
            )
            sup_patched = True
        else:
    ...
    if not sup_patched:
        print(
            "⚠️  supervisor_node patch: target cell not found (no cell has both supervisor_node and make_supervisor_node)"
        )
    ...
    for idx, cell in enumerate(cells):
        ...
        if "PATCH: force data_cleaner routing" in src:
            print(
                f"ℹ️  Cell idx {idx}: supervisor already has shortcut 0 (data_cleaner routing)"
            )
            sv0_patched = True
            break
        ...
        m0 = _re_sv0.search(
            r"^([ \t]*)# --- PATCH: force analyst routing after data cleaning ---",
            src,
            _re_sv0.MULTILINE,
        )
        if not m0:
        ...
        if new_src != src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: supervisor shortcut 0 added (initial_analysis→data_cleaner)"
            )
            sv0_patched = True
        else:
    ...
    for idx, cell in enumerate(cells):
        ...
        if "_safe_supervisor_routing_invoke" in src:
            print(
                f"ℹ️  Cell idx {idx}: supervisor_node already has routing-invoke patch"
            )
        ...
        sv_match = _re_sv.search(
            r"^([ \t]*)def supervisor_node\(state: State", new_src, _re_sv.MULTILINE
        )
        if sv_match:
            ...
            print(
                f"⚠️  Cell idx {idx}: could not find supervisor_node def for routing patch"
            )
            break
        ...
        if new_src != src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: supervisor routing protected — {call_count} routing_llm.invoke calls wrapped with retry"
            )
            sv_routing_patched = True
        else:
            print(
                f"⚠️  Cell idx {idx}: supervisor routing patch — no replacements made"
            )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if "[SHORTCUT]" in src:
            print(
                f"ℹ️  Cell idx {idx}: supervisor shortcut already has diagnostic logging"
            )
        ...
        if not m:
            ...
            break
        _di = m.group(1)
        OLD_DC_LINE = f"{_di}_dc_done = bool(state.get('data_cleaning_complete')) or (state.get('cleaning_metadata') is not None)\n"
        NEW_DC_LINE = (
        ...
        if OLD_DC_LINE in src:
            ...
            if new_src != src:
                ...
                cell["execution_count"] = None
                print(
                    f"✅ Cell idx {idx}: supervisor shortcut diagnostic logging added"
                )
                sv_diag_patched = True
        else:
            print(
                f"⚠️  supervisor diag patch: exact _dc_done pattern not found in cell {idx}"
            )
        break
    if not sv_diag_patched:
        print(
            "⚠️  supervisor shortcut diagnostic patch: not applied (shortcut may not exist yet)"
        )
    ...
    for idx, cell in enumerate(cells):
        ...
        m2 = _re_sv2.search(
            r"^([ \t]*)# --- END PATCH: force analyst routing ---",
            src,
            _re_sv2.MULTILINE,
        )
        if not m2:
        ...
        if new_src != src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: supervisor shortcuts 2+3 added (analyst→visualization, viz→report_orchestrator)"
            )
            sv2_patched = True
        else:
    ...
    if not sv2_patched:
        print(
            "⚠️  supervisor shortcut 2+3: not applied (shortcut 1 end marker not found)"
        )
    ...
    for idx, cell in enumerate(cells):
        ...
        if not m_fixd:
            print(
                f"⚠️  Fix D: 'if _dc_done and _ac_done' pattern not found in cell {idx}"
            )
            break
        _dd = m_fixd.group(1)
        ...
        FALLTHROUGH_COMMENT = (
            f"{_dd}# if _in_viz_round=True: fall through to LLM routing\n"
        )
        new_src = src.replace(OLD_D, NEW_D, 1)
        new_src = new_src.replace(
            END_VIZ_MARKER, FALLTHROUGH_COMMENT + END_VIZ_MARKER, 1
        )
        if new_src != src:
            cell["source"] = new_src
            cell["outputs"] = []
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: Fix D applied — SHORTCUT2 _in_viz_round race gate added"
            )
            fixd_patched = True
        else:
    ...
    for idx, cell in enumerate(cells):
        ...
        if not m_b2:
            print(
                f"⚠️  Fix B2: '_sc2 = int(state.get...)' pattern not found in cell {idx}"
            )
            break
        _dd2 = m_b2.group(1)
        OLD_B2 = (
        ...
        if OLD_B2 in src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: Fix B2 applied — viz escape hatch after 4 retries"
            )
            fixb2_patched = True
        else:
    ...
    for idx, cell in enumerate(cells):
        ...
        if "_in_report_round" in src:
            print(
                f"ℹ️  Cell idx {idx}: Fix V2 (_in_report_round guard) already applied"
            )
            fixv2_patched = True
            break
        ...
        if not m_fixv2:
            print(
                f"⚠️  Fix V2: 'if _vc_done and not state.get(report_generator_complete)' pattern not found in cell {idx}"
            )
            break
        _dv = m_fixv2.group(1)
        ...
        new_src = new_src.replace(
            END_REPORT_MARKER, FALLTHROUGH_COMMENT_V2 + END_REPORT_MARKER, 1
        )
        if new_src != src:
            cell["source"] = new_src
            cell["outputs"] = []
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: Fix V2 applied — SHORTCUT3 _in_report_round race gate added"
            )
            fixv2_patched = True
        else:
    ...
    for idx, cell in enumerate(cells):
        ...
        if not m_ah:
            ...
            print(
                f"⚠️  Fix AH: '_last_agent_id_sc3' not found in cell {idx} (Fix V2 may not be applied yet)"
            )
            break
        _dah = m_ah.group(1)
        ...
        if new_src != src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: Fix AH applied — SHORTCUT4 (all_done → write_output_to_file)"
            )
            fixAH_patched = True
        else:
    ...
    for idx, cell in enumerate(cells):
        ...
        if new_src != src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: P1-E supervisor config guard patched ({count} sites)"
            )
            sv_cfg_patched = True
        else:
            print(
                f"⚠️  Cell idx {idx}: P1-E config guard — no replacements (pattern not found)"
            )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if count > 0:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: P1-B stop sequence removed ({count} occurrence(s))"
            )
            sv_stop_patched = True
        else:
            print(
    ...
    for idx, cell in enumerate(cells):
        ...
        if 'routing.get("messages", [])' in src:
            print(
                f"ℹ️  Cell idx {idx}: supervisor already has P1-C messages-guard patch"
            )
        ...
        new_src = new_src.replace(
            'conv_resp["messages"]', 'conv_resp.get("messages", [])'
        )
        if new_src != src:
            cell["source"] = new_src
            cell["outputs"] = []
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: P1-C messages guard patched ({n1} routing + {n2} conv_resp)"
            )
            sv_msgs_patched = True
        else:
    ...
    for idx, cell in enumerate(cells):
        ...
        if "PATCH: P1-D model_validate" in src:
            print(
                f"ℹ️  Cell idx {idx}: supervisor already has P1-D model_validate patch"
            )
            sv_mv_patched = True
            break
        ...
        if new_src != src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: P1-D Router.model_validate fixed ({count} sites)"
            )
            sv_mv_patched = True
        else:
            print(
                f"⚠️  Cell idx {idx}: P1-D model_validate — pattern not found (may already be correct)"
            )
    ...
    for idx, cell in enumerate(cells):
        ...
        if "_routing_parse_fallback(state, routing)" in src:
            print(
                f"ℹ️  Cell idx {idx}: supervisor already has P1-G assert-fallback patch"
            )
        ...
        old_assert_28 = '                            assert isinstance(routing, Router), "Failed to parse routing result"'
        new_assert_28 = (
        ...
        old_conv_assert = '                            assert isinstance(conv_resp, ConversationalResponse), "Failed to parse routing result"'
        new_conv_assert = (
        ...
        old_assert_12 = '            assert isinstance(routing, Router), "Failed to parse routing result"'
        new_assert_12 = (
            "            if not isinstance(routing, Router):  # PATCH: P1-G fallback\n"
        ...
        old_assert_8 = '        assert isinstance(routing, Router), "Failed to parse routing result"'
        new_assert_8 = (
        ...
        if new_src != src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: P1-G assert fallback patched "
            ...
            if not found_any:
                print(
                    f"⚠️  Cell idx {idx}: P1-G assert fallback — no assert patterns found"
                )
            sv_assert_patched = (
                True  # not a blocker; asserts may have different indentation
            )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if new_src != src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: viz_worker patched (safe invoke + recursion cap=60 + recovery)"
            )
            viz_patched = True
        else:
            checks = [
    ...
    for idx, cell in enumerate(cells):
        ...
        src = join_source(cell["source"])
        if (
            "def viz_evaluator_node" not in src
            or "viz_evaluator_agent.invoke(" not in src
        ):
            continue
        if "_safe_viz_evaluator_invoke" in src:
            print(
                f"ℹ️  Cell idx {idx}: viz_evaluator_node already has safe-invoke patch"
            )
            ve_patched = True
            break
        new_src = src
        ...
        if VE_QUICKRULE_OLD in new_src:
            ...
            print(
                f"  ✅ viz_evaluator_node: quick-rule fb init + invoke replacement patched"
            )
        else:
            print(
                f"  ⚠️  viz_evaluator_node: quick-rule+invoke pattern not found — checking fallback"
            )
        ...
        if new_src != src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: viz_evaluator_node patched (safe invoke + retry + fb init fix)"
            )
            ve_patched = True
        else:
            checks = [
            ...
            for needle, label in checks:
                if needle not in src:
                    print(
                        f"⚠️  Cell idx {idx}: viz_evaluator patch - '{label}' not found"
                    )
            if new_src == src:
                print(
                    f"⚠️  Cell idx {idx}: viz_evaluator_node patch - no replacements made"
                )
        break
    if not ve_patched:
    ...
    for idx, cell in enumerate(cells):
        ...
        if FIX_T_OLD in src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: Fix T applied — viz_evaluator no-tasks Command→dict"
            )
            fixt_patched = True
        else:
            ...
            if _re.search(_pat, src, _re.DOTALL):
                ...
                cell["execution_count"] = None
                print(
                    f"✅ Cell idx {idx}: Fix T applied (fallback regex) — viz_evaluator no-tasks Command→dict"
                )
                fixt_patched = True
            else:
                print(
                    f"⚠️  Cell idx {idx}: Fix T — pattern not found in viz_evaluator_node cell"
                )
        break
    if not fixt_patched:
    ...
    for idx, cell in enumerate(cells):
        ...
        if "_FIX_U_SKIP_NONE_WRITER" in src:
            print(
                f"ℹ️  Cell idx {idx}: Fix U (migrate_thread None guard) already applied"
            )
            fixu_patched = True
            break
        if FIX_U_OLD in src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: Fix U applied — migrate_thread None-writer guard + outer try/except"
            )
            fixu_patched = True
        else:
    ...
    for idx, cell in enumerate(cells):
        ...
        if "_viz_state_update" in src:
            ...
            break
        if (
            'save_viz_for_state(state, sr, copy_mode="copy", make_relative=True).update({'
            not in src
        ):
        ...
        if OLD_E_FULL in src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: Fix E applied — viz_worker returns dict (not None)"
            )
            fixe_patched = True
        else:
            ...
            if m_loose:
                ...
                cell["execution_count"] = None
                print(
                    f"✅ Cell idx {idx}: Fix E applied (regex fallback) — viz_worker returns dict"
                )
                fixe_patched = True
            else:
    ...
    for idx, cell in enumerate(cells):
        ...
        if "_pl_logger" in src:
            print(
                f"ℹ️  Cell idx {idx}: stream_graph_output already has pipeline logger"
            )
            sg_patched = True
            break
        new_src = src
        ...
        if new_src != src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: stream_graph_output patched (PipelineLogger + stage logging)"
            )
            sg_patched = True
        else:
            checks = [
            ...
            for needle, label in checks:
                if needle not in src:
                    print(
                        f"⚠️  Cell idx {idx}: stream_graph_output patch - '{label}' not found"
                    )
            if new_src == src:
                print(
                    f"⚠️  Cell idx {idx}: stream_graph_output patch - no replacements made"
                )
        break
    if not sg_patched:
    ...
    for idx, cell in enumerate(cells):
        ...
        src = join_source(cell["source"])
        if (
            "state_vals = final_state.values" not in src
            or "Final state summary" not in src
        ):
            continue
        ...
        if new_src != src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: final-state cell patched (_log_final_state injection)"
            )
            fs_patched = True
        else:
            print(
                f"⚠️  Cell idx {idx}: final-state cell - _log_final_state replacement failed"
            )
        break
    if not fs_patched:
    ...
    for idx, cell in enumerate(cells):
        ...
        if "_INTERNAL_KWARGS" in src:
            print(
                f"ℹ️  Cell idx {idx}: MyChatOpenai already has P1-A internal-kwargs filter"
            )
            mychat_patched = True
            break
        ...
        if old_payload in src:
            ...
            print(
                f"⚠️  Cell idx {idx}: P1-A — payload pattern not found in MyChatOpenai cell"
            )
    ...
    for idx, cell in enumerate(cells):
        ...
        if "busy_timeout" in src:
            print(
                f"ℹ️  Cell idx {idx}: main graph already has P2-A SqliteSaver+WAL patch"
            )
            p2a_patched = True
            break
        ...
        if old_cp in src:
            ...
            print(
                f"⚠️  Cell idx {idx}: P2-A — MemorySaver pattern not found after StateGraph(State)"
            )
        break
    if not p2a_patched:
    ...
    for idx, cell in enumerate(cells):
        ...
        if old_stream_call in src:
            ...
            print(
                f"⚠️  Cell idx {idx}: P2-C/E — stream_graph_output call pattern not found"
            )
    ...
    for idx, cell in enumerate(cells):
        ...
        if old_supervisor_loop in src:
            ...
            print(
                f"✅ Cell idx {idx}: Fix G-1 supervisor loop cleaned (removed viz/report nodes)"
            )
        else:
            print(
                f"⚠️  Cell idx {idx}: Fix G-1 — supervisor loop pattern not found (may already be patched or different whitespace)"
            )
        ...
        if old_writer_loop in src:
            ...
            print(
                f"✅ Cell idx {idx}: Fix G-2 supervisor removed from route_to_writer loop"
            )
        else:
            print(
                f"⚠️  Cell idx {idx}: Fix G-2 — route_to_writer loop pattern not found"
            )
    ...
    for idx, cell in enumerate(cells):
        ...
        if "# Fix H:" in src:
            print(
                f"ℹ️  Cell idx {idx}: Fix H assign_viz_workers path_map already patched"
            )
            fixh_patched = True
            break
        old_pathmap = (
        ...
        if old_pathmap in src:
            ...
            print(
                f"⚠️  Cell idx {idx}: Fix H — assign_viz_workers conditional edge pattern not found"
            )
    ...
    for idx, cell in enumerate(cells):
        ...
        if fixi_old not in src:
            print(
                f"⚠️  Fix I: _guarded_open pattern not found in cell {idx} — skipping"
            )
            fixi_patched = True
            break
    ...
    for idx, cell in enumerate(cells):
        ...
        if fixj_old not in src:
            print(
                f"⚠️  Fix J: python_repl_tool invoke pattern not found in cell {idx} — skipping"
            )
            # Try a looser check
            if "sandbox_filesystem(sandbox_root" in src:
                print(
                    f"   (cell {idx} contains sandbox_filesystem but pattern mismatch)"
                )
            fixj_patched = True
            break
    ...
    for idx, cell in enumerate(cells):
        ...
        if FIXAI1_OLD_DESC in src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: Fix AI-1 applied — report_intermediate_progress description corrected"
            )
            fixai1_patched = True
        else:
            print(
                f"⚠️  Fix AI-1: report_intermediate_progress docstring pattern not found in cell {idx}"
            )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if FIXAK2_OLD in src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: Fix AK-2 applied — escalating counter in report_intermediate_progress"
            )
            fixak2_patched = True
        else:
            print(
                f"⚠️  Fix AK-2: report_intermediate_progress body pattern not found in cell {idx}"
            )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        src = join_source(cell["source"])
        if (
            "analyst_prompt_template_main" not in src
            and "analyst_prompt_template_initial" not in src
        ):
            continue
        ...
        if patched_items:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: Fix AI-2 applied — respond termination instruction added to: {', '.join(patched_items)}"
            )
            fixai2_patched = True
        break
    ...
    FIXAI2B_IA_OLD = "then output the in the following format :\n{output_format}\n\nPopulate two fields:"
    FIXAI2B_IA_NEW = (
    ...
    for idx, cell in enumerate(cells):
        ...
        if FIXAI2B_IA_OLD in new_src:
            ...
            print(
                f"⚠️  Fix AI-2b: initial analyst OUTPUT anchor not found in cell {idx}"
            )
        if FIXAI2B_RG_OLD in new_src:
            ...
            print(
                f"⚠️  Fix AI-2b: report_generator OUTPUT anchor not found in cell {idx}"
            )
        if patched_2b:
            new_src = new_src.replace(
                "analyst_prompt_template_initial = ",
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: Fix AI-2b applied — respond termination added to: {', '.join(patched_2b)}"
            )
            fixai2b_patched = True
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if FIXAI2C_DC_OLD in new_src:
            ...
            print(
                f"⚠️  Fix AI-2c: data_cleaner main OUTPUT anchor not found in cell {idx}"
            )
        if patched_2c:
            new_src = new_src.replace(
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: Fix AI-2c applied — respond termination added to: {', '.join(patched_2c)}"
            )
            fixai2c_patched = True
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if FIXAJ_DC_OLD in new_src:
            ...
            print(
                f"⚠️  Fix AJ: data_cleaner cleaning_metadata anchor not found in cell {idx}"
            )
        if patched_aj:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: Fix AJ applied — state snapshot logging for: {', '.join(patched_aj)}"
            )
            fixaj_patched = True
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if fixm_old not in src:
            print(
                f"⚠️  Fix M: 'for name, desc in recs' pattern not found in cell {idx} — trying loose match"
            )
            if "for name, desc in recs" in src:
                ...
                _m = _rem.search(
                    r"^([ \t]*)for name, desc in recs:", src, _rem.MULTILINE
                )
                ...
                if src2 != src:
                    ...
                    cell["execution_count"] = None
                    print(
                        f"✅ Cell idx {idx}: Fix M viz_orchestrator applied (loose match)"
                    )
                    fixm_patched = True
            break
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: Fix M viz_orchestrator: List[VizSpec] iteration fixed"
        )
        fixm_patched = True
        break
    if not fixm_patched:
    ...
    for idx, cell in enumerate(cells):
        ...
        src = join_source(cell["source"])
        if (
            "def visualization_orchestrator" not in src
            and "MANDATORY_SPEC_KEYS" not in src
        ):
            continue
    ...
    for idx, cell in enumerate(cells):
        ...
        if fixo2_old not in src:
            print(
                f"⚠️  Fix O-2: _normalize_viz_spec body pattern not found in cell {idx}"
            )
            fixo2_patched = True
            break
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: Fix O-2 _normalize_viz_spec model_copy + setdefault→if-not"
        )
        fixo2_patched = True
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if fixo3_loop_old in new_src:
            ...
            print(
                f"  ⚠️  Fix O-3: normalization loop pattern not found in cell {idx} — skipping loop fix"
            )
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: Fix O-3 viz_orchestrator dict/VizSpec attr + loop mutation fixed"
        )
        fixo3_patched = True
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: Fix O-4 assign_viz_workers dict coercion + empty-specs routing"
        )
        fixo4_patched = True
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: Fix AM-1 — assign_viz_workers Send now includes cleaning_metadata + analysis_insights"
        )
        fixam1_patched = True
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if fixan1_old not in src:
            print(
                f"⚠️  Fix AN-1: final_turn_msgs_list viz_worker pattern not found in cell {idx}"
            )
            fixan1_patched = True
            break
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: Fix AN-1 — viz_worker final_turn_msgs_list guard applied"
        )
        fixan1_patched = True
        break
    ...
    fixao1_new = "                    bins: Annotated[Optional[Union[int, str]], \"Number of equal-width bins (int) or NumPy estimator: 'auto','fd','doane','scott','sturges','sqrt','stone','rice'\"] = \"auto\",  # Fix AO-1: bins simplified type"
    ...
    for idx, cell in enumerate(cells):
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: Fix AO-1 — create_histogram bins simplified to Optional[Union[int, str]]"
        )
        fixao1_patched = True
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: Fix AO-2 — binrange simplified to Optional[List[float]]"
        )
        fixao2_patched = True
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: Fix AO-3 — x_range simplified to Optional[List[float]]"
        )
        fixao3_patched = True
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if fixao4_old not in src:
            print(
                f"⚠️  Fix AO-4: binwidth BinWidthSpec pattern not found in cell {idx}"
            )
            fixao4_patched = True
            break
    ...
    for idx, cell in enumerate(cells):
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: Fix AO-5 — RangeSpec changed to Optional[List[float]]"
        )
        fixao5_patched = True
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: Fix AO-7 — create_correlation_heatmap figsize simplified to List[float]"
        )
        fixao7_patched = True
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if fixap1_old not in src:
            print(
                f"⚠️  Fix AP-1: report_orchestrator RL pattern not found in cell {idx}"
            )
            fixap1_patched = True
            break
    ...
    for idx, cell in enumerate(cells):
        ...
        if fixao8_old not in src:
            ...
            if m:
                print(
                    f"⚠️  Fix AO-8: whis Tuple pattern changed in cell {idx} — partial match found at {m.start()}"
                )
            else:
    ...
    fixp1_new = '    task = state.get("individual_viz_task") or state.get("viz_spec")  # Fix P: avoid unhashable set literal'
    ...
    for idx, cell in enumerate(cells):
        ...
        if fixp1_old not in src:
            print(
                f"⚠️  Fix P-1: viz_worker task getter pattern not found in cell {idx}"
            )
            fixp1_patched = True
            break
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: Fix P-1 viz_worker unhashable set literal → safe fallback"
        )
        fixp1_patched = True
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if fixp2_old not in src:
            print(
                f"⚠️  Fix P-2: viz_worker spec.viz_instructions pattern not found in cell {idx}"
            )
            fixp2_patched = True
            break
    ...
    for idx, cell in enumerate(cells):
        ...
        if fixq_old_a in src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: Fix Q — last_agent_* fields now have use-last reducers (variant A with _report_dispatched)"
            )
            fixq_patched = True
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: Fix Q — last_agent_* fields now have use-last reducers (variant A_old with _viz_retry_count only)"
            )
            fixq_patched = True
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: Fix Q — last_agent_* fields now have use-last reducers (variant B)"
            )
            fixq_patched = True
        else:
    ...
    fixr_old_ftl = "    final_turn_msgs_list: Optional[Annotated[list[Union[AIMessage,ToolMessage]], add_messages]] # these are the final message from each agent turn"
    fixr_new_ftl = "    final_turn_msgs_list: Annotated[Optional[list[Union[AIMessage,ToolMessage]]], lambda a, b: add_messages(a or [], b or [])]  # Fix R: Annotated outermost for concurrent-safe reducers"
    fixr_old_sam = "    supervisor_to_agent_msgs: Optional[Annotated[list[SendAgentMessage], operator.add]]"
    fixr_new_sam = "    supervisor_to_agent_msgs: Annotated[Optional[list[SendAgentMessage]], lambda a, b: (a or []) + (b or [])]  # Fix R"
    ...
    for idx, cell in enumerate(cells):
        ...
        if fixr_old_ftl in new_src:
            ...
            print(
                f"✅ Cell idx {idx}: Fix R — final_turn_msgs_list Annotated outermost"
            )
            changed = True
        else:
        ...
        if fixr_old_sam in new_src:
            ...
            print(
                f"✅ Cell idx {idx}: Fix R — supervisor_to_agent_msgs Annotated outermost"
            )
            changed = True
        else:
            print(
                f"⚠️  Fix R: supervisor_to_agent_msgs pattern not found in cell {idx}"
            )
        if changed:
            cell["source"] = new_src
            cell["outputs"] = []
    ...
    for idx, cell in enumerate(cells):
        ...
        if FIXW1_OLD in src:
            ...
            print(
                f"W  Cell idx {idx}: Fix W1 — route_viz pattern not found, trying fallback"
            )
    ...
    for idx, cell in enumerate(cells):
        ...
        if FIXW2_GUARD in src:
            print(
                f"i  Cell idx {idx}: Fix W2 (quick-rule partial accept) already applied"
            )
            fixw2_patched = True
            break
        if FIXW2_OLD in src:
            ...
            cell["execution_count"] = None
            print(
                f"OK Cell idx {idx}: Fix W2 applied — viz_evaluator quick-rule accepts partial results"
            )
            fixw2_patched = True
        else:
            print(
                f"W  Cell idx {idx}: Fix W2 — quick-rule pattern not found (Fix T may not have applied)"
            )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        src = join_source(cell["source"])
        if (
            "def report_orchestrator(" not in src
            or "report_generator_agent.invoke(" not in src
        ):
            continue
        if "_safe_report_orchestrator_invoke" in src:
            print(
                f"i  Cell idx {idx}: Fix W3 (safe report_orchestrator invoke) already applied"
            )
            fixw3_patched = True
            break
        new_src = src
        ...
        if new_src != src:
            ...
            cell["execution_count"] = None
            print(
                f"OK Cell idx {idx}: Fix W3 applied — report_orchestrator safe invoke wrapper added"
            )
            fixw3_patched = True
        else:
            print(
                f"W  Cell idx {idx}: Fix W3 — pattern not found, checking what we have"
            )
            ...
            if "report_generator_agent.invoke(" in src:
                ...
                _m_w3 = _re_w3.search(
                    r"outline_response\s*=\s*report_generator_agent\.invoke\([^)]+\)",
                    src,
                )
                if _m_w3:
                    ...
                    new_src2 = src.replace(
                        old_invoke, "    outline_response = " + new_invoke, 1
                    )
                    # inject helper
    ...
    for idx, cell in enumerate(cells):
        ...
        if "report_outline: Optional[ReportOutline]" in src:
            ...
            if new_src != src:
                ...
                cell["execution_count"] = None
                print(
                    f"OK Cell idx {idx}: Fix X1 applied — report_outline use-last reducer added"
                )
                fixX1_patched = True
            else:
    ...
    for idx, cell in enumerate(cells):
        ...
        if "# W4-SC3-GATE" in src:
            print(
                f"OK Cell idx {idx}: Fix X2 superseded by W4-SC3-GATE (viz_* nodes excluded from _in_report_round)"
            )
            fixX2_patched = True
            break
        ...
        if old_x2 in src:
            ...
            cell["execution_count"] = None
            print(
                f"OK Cell idx {idx}: Fix X2 fallback applied — pre-W4 SC3 set rewritten without viz_*"
            )
            fixX2_patched = True
        else:
            print(
                f"i  Cell idx {idx}: Fix X2 — pre-W4 pattern absent (W4-SC3-GATE already in NEW_V2)"
            )
            fixX2_patched = True
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if 'update_memory_with_kind(state, state["_config"], "reports",' not in src:
            print(
                f"i  Cell idx {idx}: Fix X3 (state[_config] in update_memory_with_kind) already applied"
            )
            fixX3_patched = True
            break
        ...
        if old_x3 in src:
            ...
            cell["execution_count"] = None
            print(
                f"OK Cell idx {idx}: Fix X3 applied — state[_config] -> state.get(_config) in update_memory_with_kind"
            )
            fixX3_patched = True
        else:
            print(
                f'W  Fix X3: update_memory_with_kind(state, state["_config"]) pattern not found in cell {idx}'
            )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if "# Fix X3-rg" in src:
            print(
                f"i  Cell idx {idx}: Fix X3-rg (rg_vars template vars) already applied"
            )
        ...
        if old_x3rg in src:
            ...
            cell["execution_count"] = None
            print(
                f"OK Cell idx {idx}: Fix X3-rg applied — cleaning_metadata and all missing template vars added to rg_vars"
            )
            fixX3rg_patched = True
        else:
            print(
            ...
            if ro_start >= 0:
                ...
                cm_positions = [
                    m.start()
                    for m in _rex3rg.finditer(
                        r"    cleaning_metadata = cm  # type: ignore\n", ro_body
                    )
                ]
                if cm_positions:
                    ...
                    cell["execution_count"] = None
                    print(
                        f"OK Cell idx {idx}: Fix X3-rg applied via fallback — cleaning_metadata added to rg_vars"
                    )
                    fixX3rg_patched = True
                else:
                    print(
                        f"W  Fix X3-rg: cleaning_metadata = cm not found in report_orchestrator scope"
                    )
            else:
    ...
    for idx, cell in enumerate(cells):
        ...
        src = join_source(cell["source"])
        if (
            "def supervisor_node(" not in src
            or "planning_supervisor_llm.invoke(" not in src
        ):
            continue
        if "# Fix Y" in src:
            print(
                f"i  Cell idx {idx}: Fix Y (supervisor LLM try-except) already applied"
            )
            fixY_patched = True
            break
        ...
        patched_y_count = 0
        new_src = src
        for _m in _rey.finditer(
            r"( {8,12})(new_plan) = (planning_supervisor_llm\.invoke\([^\n]+\n)", src
        ):
            indent = _m.group(1)
        ...
        if new_src != src:
            ...
            cell["execution_count"] = None
            print(
                f"OK Cell idx {idx}: Fix Y applied — {patched_y_count} supervisor LLM invoke(s) wrapped in try-except"
            )
            fixY_patched = True
        else:
            print(
                f"W  Fix Y: planning_supervisor_llm.invoke pattern not found/replaced in cell {idx}"
            )
        break
    if not fixY_patched:
    ...
    for idx, cell in enumerate(cells):
        ...
        if "# Fix Z" in src:
            print(
                f"i  Cell idx {idx}: Fix Z (supervisor remaining LLM try-excepts) already applied"
            )
            fixZ_patched = True
            break
        ...
        if _Z7_OLD in new_src:
            ...
            print(
                f"W  Fix Z7: conversation_result=planning_supervisor_llm pattern not found in cell {idx}"
            )
        ...
        if _Z9_OLD in new_src:
            ...
            print(
                f"W  Fix Z9: conversation_result=todo_llm pattern not found in cell {idx}"
            )
    ...
    for idx, cell in enumerate(cells):
        ...
        if _FIX_AV_SENTINEL in src:
            ...
            fixAV_patched = True
            break
        _OLD_AV = 'visualization_tools.append(create_manage_memory_tool(namespace=("memories","visualization"),store= in_memory_store))\n'
        _NEW_AV = (
        ...
        if _OLD_AV in src:
            ...
            cell["execution_count"] = None
            print(
                f"OK Cell idx {idx}: Fix AV applied — manage_memory wrapped for viz tools"
            )
            fixAV_patched = True
        else:
            ...
            _av_pat = (
                r"visualization_tools\.append\(create_manage_memory_tool\([^)]+\)\)"
            )
            if _re_av.search(_av_pat, src):
                ...
                cell["execution_count"] = None
                print(
                    f"OK Cell idx {idx}: Fix AV applied (fallback: removed manage_memory from viz tools)"
                )
                fixAV_patched = True
            else:
    ...
    if not fixAV_patched:
        print(
            "W  Fix AV: visualization_tools.append(create_manage_memory_tool...) not found in any cell"
        )
    ...
    for idx, cell in enumerate(cells):
        ...
        if _OLD_AW in src:
            ...
            cell["execution_count"] = None
            print(
                f"OK Cell idx {idx}: Fix AW applied — save_viz_for_state includes supervisor fields"
            )
            fixAW_patched = True
        else:
            print(
                f"W  Fix AW: save_viz_for_state DataVisualization pattern not found in cell {idx} (may already be patched or whitespace differs)"
            )
    ...
    for idx, cell in enumerate(cells):
        ...
        if "# Fix AA" in src:
            print(
                f"i  Cell idx {idx}: Fix AA (section dict→SectionOutline) already applied"
            )
            fixAA_patched = True
            break
        _AA_OLD = (
        ...
        if _AA_OLD in src:
            ...
            cell["execution_count"] = None
            print(
                f"OK Cell idx {idx}: Fix AA applied — section dict→SectionOutline conversion"
            )
            fixAA_patched = True
        else:
    ...
    for idx, cell in enumerate(cells):
        ...
        src = join_source(cell["source"])
        if "input(" in src:
            new_src = input_pattern.sub(
                "None  # input() removed for headless execution", src
            )
            if new_src != src:
                cell["source"] = new_src
                if "outputs" in cell:
                ...
                print(
                    f"✅ Cell idx {idx}: replaced input() call(s) for headless execution"
                )
    ...
    if _fix_ax_already:
        ...
        print(
            f"OK Fix AX: post-processing PNG injection cell appended (cell idx {len(cells)-1})"
        )
    ...
    _w2_apply(
        "W2-BR7v2 (drop AgentState import)", _w2_br7v2_import_pred, _w2_br7v2_import_mut
    )

    # ---- W2-BR7v2 (State class): drop AgentState parent + inject messages field ----
    _W2_BR7v2_STATE_GUARD = (
        "# W2-BR7v2: messages with add_messages reducer (no AgentState parent)"
    )
    ...
    _w2_apply(
        "W2-BR7v2 (State class: drop AgentState + add messages)",
        _w2_br7v2_state_pred,
        _w2_br7v2_state_mut,
    )
    ...
    _w2_apply(
        "W2-STATE (reducers wired + __active_response_tool__)",
        _w2_state_pred,
        _w2_state_mut,
    )
    ...
    _W2_BR8_VG_NEW = "    viz_grade: Annotated[Optional[str], _sr_reducer]  # W2-BR8"
    _W2_BR8_VF_OLD = "    viz_feedback: Optional[str]"
    _W2_BR8_VF_NEW = "    viz_feedback: Annotated[Optional[str], _sr_reducer]  # W2-BR8"
    ...
    def _w2_br8_mut(s):
        ...
        if _W2_BR8_SR_OLD in n:
            ...
            print(
                "⚠️  W2-BR8: structured_response anchor not found "
        ...
        if _W2_BR8_VF_OLD in n:
            ...
            print(
                "⚠️  W2-BR8: viz_feedback anchor not found (may already be Annotated)"
            )
        return n
    ...
    _W2_BR6B_OLD = '        return {"viz_grade": final_grade.grade, "viz_feedback": final_grade.feedback, "viz_results": results, "viz_specs": specs,'
    _W2_BR6B_NEW = (
        "        # " + _W2_BR6B_GUARD + "\n"
    ...
    _W2_BR6B_OLD2 = '    return {"viz_grade": final_grade.grade, "viz_feedback": final_grade.feedback, "viz_results": results, "viz_specs": specs,'
    _W2_BR6B_NEW2 = (
    ...
    def _w2_bc_pred(s):
        return ("\ndata_cleaning_tools.append(report_intermediate_progress)" in s) or (
            _W2_BC_GUARD in s
        )
    ...
    _W2_BB_GUARD = (
        "# W2-BB: Fix BB replaces FIXAK2 body — threshold=3, status=error, agent-keyed"
    )
    _W2_BB_OLD = (
        '    progress_message_final = progress_message.strip() or "Empty progress message"\n'
        "    # Fix AK-2: escalating counter\n"
    ...
    _W2_RC1_ANCHORS = [
        ...
        (
            "{output_format}\nInclude: descriptive_stats",
            "AnalysisInsights",
        ),  # FIXAI2 mini analyst — anchored AFTER {of}
        (
            "{output_format}\nAlso include",
            "CleaningMetadata",
        ),  # FIXAI2 data_cleaner mini
        (
            "then output the in the following format :\n",
            "InitialDescription",
        ),  # FIXAI2b initial analyst
        (
            "Return a structured response matching:",
            "{response_tool_name}",
        ),  # FIXAI2b report_generator (DYNAMIC)
        (
            "After cleaning, summarize actions and the dataset state in the schema:",
            "CleaningMetadata",
        ),  # FIXAI2c
    ...
    def _w2_rc1c2_pred(s):
        return (
            any(
                a in s
                for a in [_RGV_NODE_OLD_OUTLINE, _RGV_NODE_OLD_PKG, _RGV_NODE_OLD_SEC]
            )
            or _W2_RC1C2_GUARD in s
        )
    ...
    def _w2_rc1c2_mut(s):
        ...
        n = s
        for old, new in [
            (_RGV_NODE_OLD_OUTLINE, _RGV_NODE_NEW_OUTLINE),
            (_RGV_NODE_OLD_PKG, _RGV_NODE_NEW_PKG),
            (_RGV_NODE_OLD_SEC, _RGV_NODE_NEW_SEC),
        ]:
            if (
                old in n
                and "response_tool_name"
                not in n[max(0, n.find(old) - 20) : n.find(old) + len(old) + 80]
            ):
                n = n.replace(old, new, 1)
        return n if n != s else None
    ...
    _W2_SAFE = [
        ...
        (
            "def _safe_initial_analysis_invoke(",
            "_outer_cfg",
            "InitialDescription",
            "initial_analysis",
            "IA",
        ...
        (
            "def _safe_visualization_invoke(",
            "_outer_vz",
        ...
        (
            "def _safe_viz_evaluator_invoke(",
            "_outer_ve",
            "VizFeedback",
            "viz_evaluator",
            "VE",
        ...
        (
            "def _safe_report_orchestrator_invoke(",
            "_outer_ro",
            "ReportOutline",
            "report_orchestrator",
            "RO",
        ...
        (
            "def _safe_report_packager_invoke(",
            "_outer_rp",
            "ReportResults",
            "report_packager",
            "RP",
    ...
    _W2_BA_STRIP_GUARD = (
        "# W2-BA-strip: SystemMessage strip + active_response_tool inject"
    )
    for _fn_anchor, _outer, _schema, _agent_name, _tag in _W2_SAFE:
        ...
        def _make_mut(
            fn_anchor=_fn_anchor, outer_anchor=outer_anchor, ins=ins, guard=guard
        ):
            def mut(s):
                if guard in s:
                    raise _W2Skip()
                ...
                if opos < 0:
                    print(
                        f"⚠️  W2-BA-strip[{guard}]: outer anchor {outer_anchor!r} not found"
                    )
                    return None
                # end of that line
                eol = s.find("\n", opos)
    ...
    _w2_apply(
        "W2-BF4 (viz_worker cleaned_dataset_description)", _w2_bf4_pred, _w2_bf4_mut
    )
    ...
    def _w2_bf5_pred(s):
        return ("def report_packager_node(" in s and _W2_BF5_OLD in s) or (
            _W2_BF5_GUARD in s
        )
    ...
    def _w2_bf5_mut(s):
        ...
        if _W2_BF5_OLD not in s:
            print(
                "⚠️  W2-BF5: report_packager rg_vars anchor not found (line wrap mismatch)"
            )
            return None
        return s.replace(_W2_BF5_OLD, _W2_BF5_NEW, 1)

    _w2_apply(
        "W2-BF5 (report_packager visualization_results)", _w2_bf5_pred, _w2_bf5_mut
    )
    ...
    _W2_DOCS2C_STATE_NEW = (
        "    # evaluator loop fields\n"
        "    "
        + _W2_DOCS2C_STATE_GUARD
        + " (last-write-wins; written only by viz_evaluator_node)\n"
    ...
    def _w2_docs2c_state_pred(s):
        return (
            "class State(TypedDict, total=False):" in s and _W2_DOCS2C_STATE_OLD in s
        ) or (_W2_DOCS2C_STATE_GUARD in s)
    ...
    _w2_apply(
        "W2-DOCS2c-state (viz_revise_count field)",
        _w2_docs2c_state_pred,
        _w2_docs2c_state_mut,
    )
    ...
    def _w2_docs2c_node_pred(s):
        return (
            "def viz_evaluator_node(" in s
            and (_W2_DOCS2C_NODE_OLD_LLM in s or _W2_DOCS2C_NODE_OLD_FB in s)
        ) or (_W2_DOCS2C_NODE_GUARD in s)
    ...
    def _w2_docs2c_node_mut(s):
        ...
        if _W2_DOCS2C_NODE_OLD_LLM in n:
            ...
            print(
                "⚠️  W2-DOCS2c-node: LLM-path anchor (W2-BR6b _w2_next_after_viz, 8-space) not found"
            )
        if _W2_DOCS2C_NODE_OLD_FB in n:
            ...
            print(
                "⚠️  W2-DOCS2c-node: fallback-path anchor (W2-BR6b _w2_next_after_viz, 4-space) not found"
            )
    ...
    _w2_apply(
        "W2-DOCS2c-node (cap enforcement + viz_revise_count emit)",
        _w2_docs2c_node_pred,
        _w2_docs2c_node_mut,
    )
    ...
    def _w2_docs2c_prompt_pred(s):
        return (
            "viz_evaluator_prompt_template = ChatPromptTemplate.from_messages" in s
            and _W2_DOCS2C_PROMPT_OLD in s
        ) or (_W2_DOCS2C_PROMPT_GUARD in s)
    ...
    _w2_apply(
        "W2-DOCS2c-prompt (revise-budget paragraph)",
        _w2_docs2c_prompt_pred,
        _w2_docs2c_prompt_mut,
    )
    ...
    _W2_DOCS2C_RUN_NEW = (
        ...
        '                "memories" : enhanced_retrieve_mem(state),  "visualization_results": results,\n'
        '                "user_prompt": user_prompt,\n'
        '                "viz_revise_count": int(state.get("viz_revise_count", 0) or 0),  '
        + _W2_DOCS2C_VARS_GUARD
        + " (runtime)\n"
        '                "analysis_insights": state.get("analysis_insights", None), "cleaned_dataset_description": state.get("cleaned_dataset_description", None)}'
    )
    ...
    def _w2_docs2c_vars_pred(s):
        return (
            (_W2_DOCS2C_INIT_OLD in s)
            or (_W2_DOCS2C_RUN_OLD in s)
            or (_W2_DOCS2C_VARS_GUARD in s)
        )
    ...
    _w2_apply(
        "W2-DOCS2c-vars (init_viz_vars + vis_vars)",
        _w2_docs2c_vars_pred,
        _w2_docs2c_vars_mut,
    )
    ...
    _W2_BR8D_VG_OLD = "    viz_grade: Annotated[Optional[str], _sr_reducer]  # W2-BR8"
    _W2_BR8D_VG_NEW = "    viz_grade: Optional[str]  " + _W2_BR8D_GUARD
    _W2_BR8D_VF_OLD = (
        "    viz_feedback: Annotated[Optional[str], _sr_reducer]  # W2-BR8"
    )
    _W2_BR8D_VF_NEW = "    viz_feedback: Optional[str]  " + _W2_BR8D_GUARD
    ...
    def _w2_br8d_mut(s):
        ...
        if _W2_BR8D_VG_OLD in n:
            ...
            print(
                "⚠️  W2-BR8d: viz_grade Annotated anchor not found (W2-BR8 may not have run)"
            )
    ...
    _w2_apply(
        "W2-BR8d (revert viz_grade/viz_feedback reducer)", _w2_br8d_pred, _w2_br8d_mut
    )
    ...
    def _w2_emergency_mut(s):
        ...
        if _W2_EMERGENCY_OLD not in s:
            print(
                "⚠️  W2-EMERGENCY: workers fan-in loop anchor not found (source drift?)"
            )
            return None
        return s.replace(_W2_EMERGENCY_OLD, _W2_EMERGENCY_NEW, 1)

    _w2_apply(
        "W2-EMERGENCY (EMERGENCY_MSG edge to supervisor)",
        _w2_emergency_pred,
        _w2_emergency_mut,
    )
    ...
    _w2_apply(
        "W2-BF6[DC] (static data_cleaner system_prompt)", _w2_bf6dc_pred, _w2_bf6dc_mut
    )
    ...
    _w2_apply(
        "W2-BF6[AN] (static analyst system_prompt)", _w2_bf6an_pred, _w2_bf6an_mut
    )
    ...
    _w2_apply(
        "W2-REC6 (unknown-tool guard helper)", _w2_rec6_helper_pred, _w2_rec6_helper_mut
    )
    ...
    for _tag, _old_mw, _tools_var, _aname, _schema in _W2_REC6_TARGETS:
        ...
        def _make_mut(old_mw=_old_mw, guard=_guard, new_mw=_new_mw):
            def _m(s):
                ...
                if old_mw not in s:
                    print(
                        f"⚠️  W2-REC6[{guard.split(']')[0].split('[')[1]}]: middleware anchor `{old_mw}` not found"
                    )
                    return None
        ...
        _w2_apply(
            f"W2-REC6[{_tag}] (unknown-tool guard wire)", _make_pred(), _make_mut()
        )
    ...
    _W2_DOCS1_GUARD = (
        "# W2-DOCS1: structured_response renamed to final_structured_output"
    )
    _W2_DOCS1_DECL_OLD = (
        "    structured_response: Annotated[Optional[BaseNoExtrasModel], _sr_reducer]  "
        "# W2-BR8: prefer non-None last write"
    ...
    def _w2_docs1_pred(s):
        return (
            (_W2_DOCS1_DECL_OLD in s)
            or (_W2_DOCS1_WRITE_OLD in s)
            or (_W2_DOCS1_GUARD in s)
        )
    ...
    def _w2_docs1_mut(s):
        ...
        if _W2_DOCS1_WRITE_OLD in n:
            n = n.replace(
                _W2_DOCS1_WRITE_OLD, _W2_DOCS1_WRITE_NEW
            )  # all occurrences in cell
    ...
    def _w2_docs2a_mut(s):
        ...
        new_local = (
            'else ChatOpenAI(timeout=600, max_retries=2, base_url=f"{ngrok_url}/v1"'
        )
    ...
    def _w2_br8b_mut(s):
        ...
        if _W2_BR8B_VT_OLD in n:
            ...
            if (
                loose in n
                and "Annotated[List[str]" not in n.split(loose, 1)[1].split("\n", 1)[0]
            ):
                n = n.replace(
                    loose,
                ...
                print(
                    "⚠️  W2-BR8b: viz_tasks anchor not found (may already be Annotated)"
                )
        n = n.replace(
            "class State(TypedDict, total=False):",
            f"{_W2_BR8B_GUARD}\nclass State(TypedDict, total=False):",
    ...
    def _w2_br8c_mut(s):
        ...
        if _W2_BR8C_RR_OLD in n:
            ...
            print(
                "⚠️  W2-BR8c: report_results anchor not found (may already be Annotated)"
            )
        n = n.replace(
            "class State(TypedDict, total=False):",
            f"{_W2_BR8C_GUARD}\nclass State(TypedDict, total=False):",
    ...
    _w2_apply(
        "W2-DOCS3 (drop dead create_react_agent import)", _w2_docs3_pred, _w2_docs3_mut
    )
    ...
    _W4_SUPLIMIT_STATE_INSERT = (
        "    "
        + _W4_SUPLIMIT_STATE_GUARD
        + " (operator.add reducer; bounded by IDD_SUPERVISOR_MAX_TURNS, default 30)\n"
    ...
    _w2_apply(
        "W4-SUPLIMIT (state field _supervisor_turn_count)",
        _w4_suplimit_state_pred,
        _w4_suplimit_state_mut,
    )
    ...
    for _idx, _cell in enumerate(cells):
        ...
        if _W4_SUPLIMIT_NODE_GUARD in _src:
            print(
                f"i  Cell idx {_idx}: W4-SUPLIMIT (supervisor cap-check) already applied"
            )
            _w4sl_patched = True
            break
        _m_sl = _re_w4sl.search(
            r"^([ \t]*)def supervisor_node\(state: State, config: RunnableConfig\):\n",
            _src,
            _re_w4sl.MULTILINE,
        )
        ...
        _ret_pat = _re_w4sl.compile(
            r"^([ \t]*)return supervisor_node\s*$", _re_w4sl.MULTILINE
        )
        if _ret_pat.search(_new_src):
            _new_src = _ret_pat.sub(
                _wrap_block + r"\1return supervisor_node", _new_src, count=1
            )
        else:
            print(
                f"W  W4-SUPLIMIT: 'return supervisor_node' anchor not found in cell {_idx}; wrap NOT applied"
            )
        if _new_src != _src:
            ...
            _cell["execution_count"] = None
            print(
                f"OK Cell idx {_idx}: W4-SUPLIMIT applied — cap-check + decorator wrap"
            )
            _w4sl_patched = True
        break
    ...
    def _w4_ve_safefb_pred(s):
        return ("def viz_evaluator_node(" in s) and (
            _W4_VE_SAFEFB_OLD in s or _W4_VE_SAFEFB_GUARD in s
        )
    ...
    _w2_apply(
        "W4-VE-SAFEFB (safe .get on fb structured_response)",
        _w4_ve_safefb_pred,
        _w4_ve_safefb_mut,
    )
    ...
    _W4_VE_OUTERGUARD_GUARD = (
        "# W4-VE-OUTERGUARD: outer try/except around viz_evaluator_node"
    )
    ...
    _w2_apply(
        "W4-VE-OUTERGUARD (outer try/except viz_evaluator_node)",
        _w4_ve_og_pred,
        _w4_ve_og_mut,
    )
    ...
    _W5_FW_ROUTE_NEW = (
        'already_wrote = bool(state.get("file_writer_complete"))  # W5-FW-ROUTE'
    )
    ...
    _w2_apply(
        "W5-FW-ROUTE (gate route_to_writer on file_writer_complete)",
        _w5_fw_route_pred,
        _w5_fw_route_mut,
    )
    ...
    _w2_apply(
        "W5-FW-STAGE (file_writer STAGE markers)", _w5_fw_stage_pred, _w5_fw_stage_mut
    )
    ...
    _W6_FW_PDF_NEW = (
        ...
        "    "
        + _W6_FW_PDF_GUARD
        + ": auto-generate PDF from final HTML if LLM omitted it\n"
        "    try:\n"
    ...
    _w2_apply(
        "W6-FW-PDF-FORCE (deterministic PDF generation)",
        _w6_fw_pdf_pred,
        _w6_fw_pdf_mut,
    )
    ...
    _w2_apply(
        "W7-SR-ALIGN (mirror AgentState structured_response annotation)",
        _w7_sr_align_pred,
        _w7_sr_align_mut,
    )
    ...
    _w2_apply(
        "W9-SR-DROP (remove structured_response from supervisor State)",
        _w9_sr_drop_pred,
        _w9_sr_drop_mut,
    )
    ...
    _w2_apply(
        "W8-VW-NOSR (drop structured_response write from viz_worker)",
        _w8_vw_pred,
        _w8_vw_mut,
    )
    ...
    _w2_apply(
        "W10-PDF-POST (post-graph deterministic PDF)",
        _w10_pdf_post_pred,
        _w10_pdf_post_mut,
    )
    ...
    for idx, cell in enumerate(cells):
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W11-RO restored report_orchestrator agent invocation"
        )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if start < 0 or end < 0:
            print(
                f"⚠️  W11-SW: deterministic section block anchors not found in cell {idx}"
            )
            break
        new_src = src[:start] + _W11_SW_PREP + src[end:]
        invoke_line = "    msg = report_section_agent.invoke({\n"
        if (
            invoke_line in new_src
            and "STATE report_section_agent.invoke.start" not in new_src
        ):
            new_src = new_src.replace(
                invoke_line,
                "    try:\n"
        ...
        if content_anchor in new_src and "_w11_scaffold_phrases" not in new_src:
            new_src = new_src.replace(
                content_anchor, scaffold_check + content_anchor, 1
            )
        _W11_SW_RETURN_OLD = (
            "    return {\n"
    ...
    for idx, cell in enumerate(cells):
        ...
        if ret_pos >= 0:
            new_src = (
                new_src[:ret_pos]
                + _W11_RP_RETURN_NEW
                + new_src[ret_pos + len(_W11_RP_RETURN_OLD) :]
            )
        else:
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W11-RP requires report_packager_agent before rendering"
        )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if not pat.search(src):
            print(
                f"⚠️  W11B-S1: report_draft annotation anchor not found in State cell {idx}"
            )
            break
        fields = (
            "\\1"
    ...
    for idx, cell in enumerate(cells):
        ...
        if start < 0 or end < 0:
            print(
                f"⚠️  W11B-SW: robust deterministic section anchors not found in cell {idx}"
            )
            break
        prep = (
            "    expected_viz = section.expected_figures if section else []\n"
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W11B-SW removed deterministic section prose and added markers"
        )
        break
    ...
    _W12_DC_GUARD = (
        "# W12-DC: data_cleaner update helper accepts optional result_mapping"
    )
    for idx, cell in enumerate(cells):
        if cell.get("cell_type") != "code":
            continue
        ...
        if _W12_DC_GUARD in src:
            break
        new_src = src
        old_sig = "def _data_cleaner_update_from_metadata(state: State, cleaning_metadata: CleaningMetadata, result_messages: Optional[list] = None) -> dict:"
        new_sig = "def _data_cleaner_update_from_metadata(state: State, cleaning_metadata: CleaningMetadata, result_messages: Optional[list] = None, result_mapping: Optional[dict] = None) -> dict:"
    ...
    for idx, cell in enumerate(cells):
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W12C-DC routed data_cleaner invoke through safe wrapper"
        )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        new_src = src.replace(
            "def route_to_writer(state)", helper + "def route_to_writer(state)", 1
        )
        new_src = new_src.replace(
        ...
        if new_src != src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: W13-VIZREADY routes use visualization_results after viz_join"
            )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if old in src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: W13-RO added structured final-hop for report_orchestrator"
            )
        else:
            print(
                f"⚠️  W13-RO: report_orchestrator invoke anchor not found in cell {idx}"
            )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        src = join_source(cell["source"])
        if (
            "def report_orchestrator(state: State):" not in src
            or "with_structured_output(ReportOutline)" not in src
        ):
            continue
        ...
        if old in src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: W13B-RO switched ReportOutline final-hop to parsed LLM JSON"
            )
        else:
    ...
    for idx, cell in enumerate(cells):
        ...
        if (
            "def report_orchestrator(state: State):" not in src
            or "_ro_outline = ReportOutline.model_validate(_ro_json.loads(_ro_raw))"
            not in src
        ):
            continue
    ...
    for idx, cell in enumerate(cells):
        ...
        if (
            "def report_orchestrator(state: State):" not in src
            or "_ro_payload = _ro_json.loads(_ro_raw2)" not in src
        ):
            continue
    ...
    for idx, cell in enumerate(cells):
        ...
        if (
            "def report_orchestrator(state: State):" not in src
            or "def _ro_load_mapping(_txt):" not in src
        ):
            continue
    ...
    for idx, cell in enumerate(cells):
        ...
        if (
            "def report_orchestrator(state: State):" not in src
            or "outline_response = report_generator_agent.invoke(" not in src
        ):
            continue
    ...
    for idx, cell in enumerate(cells):
        ...
        if (
            "def report_orchestrator(state: State):" not in src
            or "ReportOutline.model_validate(_ro_payload)" not in src
        ):
            continue
        ...
        if old in src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: W13G-RO normalizes outline payload before validation"
            )
        else:
    ...
    for idx, cell in enumerate(cells):
        ...
        src = join_source(cell["source"])
        if (
            "def section_worker(state: State):" not in src
            or "report_section_agent.invoke({" not in src
        ):
            continue
        if _W13H_SW_GUARD in src:
            break
        old_rendered = '    rendered = system_message_content.format_messages(messages=[HumanMessage(content=user_prompt, name="user"),newest_msg,AIMessage(content=default_instruction,name="supervisor")], **rg_vars)\n'
        new_rendered = (
            f"    # {_W13H_SW_GUARD}\n"
        ...
        if old_invoke in src:
            ...
            print(
                f"⚠️  W13H-SW: report_section_agent.invoke anchor not found in cell {idx}"
            )
        cell["source"] = src
        cell["outputs"] = []
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W13H-SW compacts section prompts and retries transient API failures"
        )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        src = join_source(cell["source"])
        if (
            "def analyst_node(state: State):" not in src
            or "analyst_agent.invoke(" not in src
        ):
            continue
        ...
        if old in src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: W13I-AN retries analyst on orphan tool-output BadRequest"
            )
        else:
    ...
    for idx, cell in enumerate(cells):
        ...
        if (
            "def report_orchestrator(state: State):" not in src
            or "def section_worker(state: State):" not in src
        ):
            continue
        if _W13J_SW_GUARD in src:
            break
        bad_start = src.find(
            "    # # W13H-SW: compact prompt plus transient API retry\n"
        )
        bad_end = src.find(
        ...
        section_render = '    rendered = system_message_content.format_messages(messages=[HumanMessage(content=user_prompt, name="user"),newest_msg,AIMessage(content=default_instruction,name="supervisor")], **rg_vars)\n'
        compact_render = (
            f"    # {_W13J_SW_GUARD}\n"
        ...
        if section_render in sec:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: W13J-SW restored report_orchestrator render and scoped compact prompt to section_worker"
            )
        else:
    ...
    for idx, cell in enumerate(cells):
        ...
        src = join_source(cell["source"])
        if (
            "def _deterministic_viz_update_from_spec" not in src
            or "bins=int(spec.bins or 20)" not in src
        ):
            continue
    ...
    for idx, cell in enumerate(cells):
        ...
        if (
            "def section_worker(state: State):" not in src
            or "_w13h_reason not in" not in src
        ):
            continue
        ...
        if old in src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: W13L-SW recovers recursive section agents via direct LLM final-hop"
            )
        else:
    ...
    for idx, cell in enumerate(cells):
        ...
        if _W13M_RJ_GUARD in src:
            ...
            if (
                ...
                cell["execution_count"] = None
                print(
                    f"✅ Cell idx {idx}: W13M-RJ removed legacy viz_join written_sections reset"
                )
            break
        old = (
        ...
        if old in src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: W13M-RJ preserves written_sections through report_join"
            )
            src = join_source(cell["source"])
            if (
                '"written_sections": None,  # W2-BR2: also flush from viz path (no-op if not present)\n'
    ...
    for idx, cell in enumerate(cells):
        ...
        src = join_source(cell["source"])
        if (
            "def file_writer_node(state: State):" not in src
            or "STATE file_writer.blocked reason=insufficient_report_content" not in src
        ):
            continue
        ...
        if old in src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: W13N-FW uses assembled report content for final file writing"
            )
        else:
    ...
    for idx, cell in enumerate(cells):
        ...
        if _W13N_RT_GUARD in src:
            break
        old = "    # Optional: guard against typos\n" "    allowed: set[str] = {\n"
        new = (
            f"    # {_W13N_RT_GUARD}\n"
        ...
        if old in src:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: W13N-RT routes final supervisor state to file_writer/FINISH"
            )
        else:
    ...
    for idx, cell in enumerate(cells):
        ...
        src = join_source(cell["source"])
        if (
            "def report_packager_node(state: State):" not in src
            or "_w11_pkg_result = report_packager_agent.invoke(" not in src
        ):
            continue
        ...
        invoke_block_indented = (
            ...
            "    try:\n"
            + invoke_block.replace(
                "    _w11_pkg_result = ", "        _w11_pkg_result = ", 1
            )
        )
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W13P-RP recovers recursive report_packager via direct LLM final-hop"
        )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        src = join_source(cell["source"])
        if (
            "def create_report_generator_agent(" not in src
            or "tools=report_generator_tools" not in src
        ):
            continue
        ...
        if old_map not in src:
            print(
                f"⚠️  W13Q-RG-ROLES: output_format_map anchor not found in cell {idx}"
            )
            break
        ...
        if old_tools not in src:
            print(
                f"⚠️  W13Q-RG-ROLES: tool_descriptions anchor not found in cell {idx}"
            )
            break
        ...
        if old_agent_tools not in src:
            print(
                f"⚠️  W13Q-RG-ROLES: create_agent tools anchor not found in cell {idx}"
            )
            break
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W13Q-RG-ROLES applied — report agents now use role-specific tools/contracts"
        )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        old_allowed_outline = '            "allowed_tool_names": {"read_file", "list_available_files", "list_visualizations", "get_visualization"},\n'
        new_allowed_outline = '            "allowed_tool_names": {"list_visualizations", "get_visualization"},\n'
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W13R report generator prompt now renders role contract and avoids binary reads"
        )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        src = join_source(cell["source"])
        if (
            "def _gather_from_state(" not in src
            or "def list_visualizations(" not in src
        ):
            continue
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W13R visualization listing now sees structured viz state"
        )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if (
            "def viz_evaluator_node(" not in src
            or "_safe_viz_evaluator_invoke(viz_evaluator_agent" not in src
        ):
            continue
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W13R viz_evaluator passes structured visualization state to tools"
        )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if (
            "def file_writer_node(state: State):" not in src
            or "result = file_writer_agent.invoke(" not in src
        ):
            continue
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W13S file_writer final phase returns no-write ListOfFiles manifest"
        )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        src = join_source(cell["source"])
        if (
            "FINAL viz=" not in src
            or '_viz_items = state_vals.get("viz_results", []) or []' not in src
        ):
            continue
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W13S final marker counts persisted visualization_results"
        )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W13S image injector uses relative paths and skips existing images"
        )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if (
            "def section_worker(state: State):" not in src
            or "Correlation insights:" not in src
        ):
            continue
        ...
        if old_line in src:
            ...
            print(
                f"⚠️  W13S-NUM: compact section prompt anchor not found in cell {idx}"
            )
        old_direct = "                    f\"Correlation insights: {_w13j_short(getattr(insights, 'correlation_insights', ''), 900) if '_w13j_short' in locals() else str(getattr(insights, 'correlation_insights', ''))[:900]}\",\n"
        new_direct = (
            old_direct
            + '                    "Preserve exact numeric evidence from AnalysisInsights. When citing correlations, write them in the form r = -0.1607 (not approximately or rounded away).",\n'
        )
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W13S section prompt preserves exact numeric evidence"
        )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        src = join_source(cell["source"])
        if (
            "def create_initial_analysis_agent" not in src
            or "ToolStrategy(InitialDescription)" not in src
        ):
            continue
        ...
        if old_prompt not in src:
            print(
                f"⚠️  W13T-IA: initial analysis prompt anchor not found in cell {idx}"
            )
            break
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W13T-IA scoped initial_analysis factory prompt/tools"
        )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if (
            "def initial_analysis_node" not in src
            or "def data_cleaner_node" not in src
            or "def analyst_node" not in src
        ):
            continue
        ...
        if old_ia_recovery in src:
            ...
            print(
                f"⚠️  W13T-NODES: initial_analysis recovery anchor not found in cell {idx}"
            )
        ...
        if old_ia_structured in src:
            ...
            print(
                f"⚠️  W13T-NODES: initial_analysis structured log anchor not found in cell {idx}"
            )
        ...
        if old_dc_update in src:
            ...
            print(
                f"⚠️  W13T-NODES: data_cleaner update-log anchor not found in cell {idx}"
            )
        ...
        if old_dc_log in src:
            ...
            print(
                f"⚠️  W13T-NODES: data_cleaner log rename anchor not found in cell {idx}"
            )
    ...
    for idx, cell in enumerate(cells):
        ...
        if (
            "def analyst_node(state: State):" not in src
            or "STATE analyst.recovered reason=orphan_tool_output retry=compact"
            not in src
        ):
            continue
        if _W13U_AN_GUARD in src:
            break
        anchor = '    analyst_configurable = dict((state.get("_config") or {}).get("configurable", {}))\n'
        insert = (
            f"    # {_W13U_AN_GUARD}\n"
            '    _w13u_prompt = "\\n".join([\n'
            "        user_prompt,\n"
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W13U-AN isolates analyst first attempt and fail-closes orphan retry"
        )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if _W13V_SW_GUARD in src:
            break
        changed = False
        section_tools_old = '            "allowed_tool_names": {"list_visualizations", "get_visualization"},\n'
        section_tools_new = '            "allowed_tool_names": set(),  # W13V-SW: Section context is provided in invoke state; no runtime tools.\n'
        ...
        if first >= 0:
            ...
            if second >= 0:
                src = (
                    src[:second]
                    + section_tools_new
                    + src[second + len(section_tools_old) :]
                )
                changed = True
        no_tools_old = (
            "    if not role_tools:\n"
        ...
        if changed:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: W13V-SW removed section runtime tools and fail-closes section recursion"
            )
        else:
    ...
    for idx, cell in enumerate(cells):
        ...
        if (
            "def create_report_generator_agent(" not in src
            or _W13V2_FACTORY_GUARD in src
        ):
            continue
        changed = False
        ...
        if sec_start >= 0:
            ...
            if pos >= 0:
                src = (
                    src[:pos]
                    + '            "allowed_tool_names": set(),  # W13V2-SW: Section context is supplied in invoke state.\n'
                    + src[pos + len(old) :]
                )
                changed = True
        old = (
        ...
        if changed:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: W13V2-SW report factory gives sections no runtime tools"
            )
        else:
    ...
    for idx, cell in enumerate(cells):
        ...
        if (
            "def section_worker(state: State):" not in src
            or _W13V2_RECOVERY_GUARD in src
        ):
            continue
        old = (
            '            if _w13h_reason == "GraphRecursionError":\n'
    ...
    for idx, cell in enumerate(cells):
        ...
        if (
            "def create_report_generator_agent(" not in src
            and "def report_packager_node(state: State):" not in src
        ):
            continue
        ...
        if changed:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: W13W-RP removed report_packager runtime file-write path"
            )
        else:
    ...
    for idx, cell in enumerate(cells):
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W13X-FINAL-REPORT-NAMES writes canonical final_report artifacts"
        )
        break

    # --- W13Z-FW-VIZ-PATHS: include report-referenced visualization copies in final manifest ---
    _W13Z_FW_VIZ_GUARD = (
        "# W13Z-FW-VIZ-PATHS: include report-referenced visualization paths"
    )
    for idx, cell in enumerate(cells):
        if cell.get("cell_type") != "code":
            continue
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W13Z-FW-VIZ-PATHS added report-referenced visualization paths to final manifest"
        )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if (
            "def create_initial_analysis_agent" not in src
            and "def create_report_generator_agent(" not in src
        ):
            continue
        ...
        if changed:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: W14A-PROMPT-PARITY restored original-style stable prompt wording"
            )
    ...
    for idx, cell in enumerate(cells):
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W14B-REPORT-HEADINGS normalizes duplicate report headings"
        )
        break

    # --- W14C-NO-MARKER-TXT: prevent helper/sample/cleaner marker text artifacts ---
    _W14C_NO_MARKER_GUARD = (
        "# W14C-NO-MARKER-TXT: no sample/cleaner marker text artifacts"
    )
    for idx, cell in enumerate(cells):
        if cell.get("cell_type") != "code":
            continue
        ...
        if old not in src:
            print(
                f"⚠️  W14C-NO-MARKER-TXT create_sample anchor not found in cell {idx}"
            )
            break
        src = src.replace(old, new, 1)
        cell["source"] = src
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W14C-NO-MARKER-TXT create_sample no longer writes outline.txt"
        )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if changed:
            ...
            cell["execution_count"] = None
            print(
                f"✅ Cell idx {idx}: W14C-NO-MARKER-TXT scoped data_cleaner away from generic file tools"
            )
        else:
            print(
                f"⚠️  W14C-NO-MARKER-TXT data_cleaner anchors not found in cell {idx}"
            )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if (
            "def file_writer_node(state: State):" not in src
            or _W14D_FW_PATHS_GUARD in src
        ):
            continue
        old = (
            '                    _w13z_path = (_w13z_html_path.parent / PathlibPath(_w13z_ref.replace("/", _w13z_os.sep))).resolve()\n'
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W14D-FW-PATHS resolves report image refs for final manifest"
        )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if (
            "def file_writer_node(state: State):" not in src
            or _W14E_FW_NORMALIZE_GUARD in src
        ):
            continue
        old = (
            "        _w13s_existing_paths = set(_w13s_report_paths + _w13s_viz_paths)\n"
        ...
        if old not in src:
            ...
            break
        src = src.replace(old, new, 1)
        old = "                _w13s_match = next((str(_p) for _p in _w13s_existing_paths if PathlibPath(str(_p)).name == _w13s_base), None)\n"
        new = (
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W14E-FW-NORMALIZE normalizes final manifest returned paths"
        )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if (
            "def report_packager_node(state: State):" not in src
            or _W14F_READABILITY_GUARD in src
        ):
            continue
        old = (
            "    draft = _normalize_report_headings(draft, title)\n"
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W14F-READABILITY-POLISH removes scaffold lead-ins from report draft"
        )
        break
    ...
    for idx, cell in enumerate(cells):
        ...
        if (
            "def file_writer_node(state: State):" not in src
            or _W14G_FW_DEFERRED_GUARD in src
        ):
            continue
        old = (
            "        _w13s_report_file_results = []\n"
        ...
        cell["execution_count"] = None
        print(
            f"✅ Cell idx {idx}: W14G-FW-DEFERRED-VIZ suppresses false copied-viz path warnings"
        )
        break
