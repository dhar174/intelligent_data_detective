...
class TestPromptTemplateFormatting:
    ...
    def test_double_braces_render_as_literal_placeholder(self):
        template = ChatPromptTemplate.from_messages(
            [("system", "Objective: {{user_prompt}}")]
        )
        content = template.format_messages(user_prompt="Analyze this dataset")[
            0
        ].content
        assert "{user_prompt}" in content
        assert "Analyze this dataset" not in content
