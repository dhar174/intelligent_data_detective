...
10. Log structured output calls using allowlisted metadata (e.g., model name, latency, token counts, schema version, error categories, and validation outcome) with strict redaction of sensitive content. Avoid persisting raw input payloads or raw model responses by default to prevent leaking PII or secrets; require explicit opt-in capture with sanitization/masking for detailed debug tracing.
