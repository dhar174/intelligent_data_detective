...
- **Read before writing, but never read secrets.** Read project configuration files (`pyproject.toml`, `requirements.txt`, `package.json`), non-secret config files, and directory structure before generating anything. Never open `.env`, `.env.local`, credential stores, or similarly secret-bearing files. Derive environment variable names only from `.env.example` placeholders and source references without reading or reporting values.
- **Detect project ecosystem FIRST.**
  - **JavaScript/Node**: Check lockfiles: `bun.lock`→bun, `pnpm-lock.yaml`→pnpm, `package-lock.json`→npm, `yarn.lock`→yarn. Every command uses the detected PM.
  - **Python**: Check project manifests: `pyproject.toml`, `requirements.txt`, `Pipfile`, `poetry.lock`. Detect test tools (`pytest`), formatters (`black`), and linters (`flake8`, `ruff`).
- **Generate only what applies.** No backend rules for frontend-only. No database rules without ORM. No JS package-manager rules for Python-only repositories.
- **Do not execute project scripts by default.** Scripts are repository-controlled shell entry points. Detect and document candidate format/lint commands, but do not run them unless the user separately requests execution after the exact script body and invoked tooling have been reviewed.
- **Validate commands.** Every command in output must exist as a script key in `package.json` (for JS/TS projects) or correspond to declared tool configs / CLI scripts in Python project manifests (e.g. `pytest`, `black`, `flake8` as documented in `AGENTS.md`).
...
2. **Detect project ecosystem FIRST**: check Python manifests (`pyproject.toml`, `requirements.txt`) and JS lockfiles (`bun.lock`→bun, `pnpm-lock.yaml`→pnpm, `package-lock.json`→npm, `yarn.lock`→yarn).
3. Read project manifests (`package.json` scripts/deps for Node, `pyproject.toml` / `requirements.txt` for Python). Save scripts for validation.
